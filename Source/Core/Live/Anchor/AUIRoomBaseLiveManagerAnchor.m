...
@synthesize onSetupBeautyBlock;
@synthesize onReceivedCustomLike;
...
- (void)setupLivePusher {
    ...
    self.livePusher.onSetupBeautyBlock = self.onSetupBeautyBlock;
    if (self.roomV) {
        self.livePusher.beautyController = [AUIRoomBeautyManager createController:self.roomV contextMode:self.liveService.liveInfoModel.mode == AUIRoomLiveModeLinkMic];
    }
    self.isLiving = NO;
}
...
- (void)queryListGroupUserWithGroupID:(NSString *)groupId sortType:(NSInteger)sortype pageNum:(NSInteger)pageNum pageSize:(NSInteger)pageSize
    ...
    [self.liveService queryListGroupUserWithGroupID:self.liveInfoModel.chat_id sortType:1 pageNum:1 pageSize:100 onSuccess:onSuccess onFailure:onFailure];
}
...
- (void)queryGroupUserByIdListWithGroupID:(NSString *)groupId userIdList:(NSArray<NSString*>*)userIdList
                                ...
                                onFailure:(void (^)(AVCIInteractionError* error))onFailure {
    [self.liveService queryGroupUserByIdListWithGroupID:groupId userIdList:userIdList onSuccess:onSuccess onFailure:onFailure];
}
...
- (void)setOnReceivedCustomLike:(void (^)(AUIRoomUser * _Nonnull, AUIRoomCustomLikeModel * _Nonnull))onReceivedCustomLike {
    self.liveService.onReceivedCustomLike = onReceivedCustomLike;
}
