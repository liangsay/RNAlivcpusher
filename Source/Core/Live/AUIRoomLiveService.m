...
typedef NS_ENUM(NSUInteger, AUIRoomMessageType) {
    ...
    AUIRoomMessageTypeCustomLike = 30002,
};
...
@implementation AUIRoomCustomLikeModel
- (instancetype)initWithData:(NSDictionary *)data {
    self = [super init];
    if (self) {
        _messageId = [data objectForKey:@"messageId"];
        _groupId = [data objectForKey:@"groupId"];
        _msgType = [data objectForKey:@"msgType"];
        _userId = [data objectForKey:@"userId"];
        _userNick = [data objectForKey:@"userNick"];
        _userAvatar = [data objectForKey:@"userAvatar"];
    }
    return self;
}
...
//MARK: 自定义点赞消息
- (void)sendCustomLike:(AUIRoomCustomLikeModel *)like completed:(void(^)(BOOL))completed {
    if (!self.isJoined || self.isAnchor) {
        if (completed) {
            completed(NO);
        }
    ...
    [self sendData:like type:AUIRoomMessageTypeCustomLike receiverId:self.liveInfoModel.anchor_id completed:completed];
}
...
- (void)queryListGroupUserWithGroupID:(NSString *)groupId sortType:(int32_t)sortype pageNum:(int32_t)pageNum pageSize:(int32_t)pageSize
    ...
    [self.messageService queryListGroupUserWithGroupID:self.liveInfoModel.chat_id sortType:1 pageNum:1 pageSize:100 onSuccess:onSuccess onFailure:onFailure];
}
...
- (void)queryGroupUserByIdListWithGroupID:(NSString *)groupId userIdList:(NSArray<NSString*>*)userIdList
                                ...
                                onFailure:(void (^)(AVCIInteractionError* error))onFailure{
    [self.messageService queryGroupUserByIdListWithGroupID:groupId userIdList:userIdList onSuccess:onSuccess onFailure:onFailure];
}
...
- (void)onCommandReceived:(AUIMessageModel *)message {
    ...
    if (message.msgType == AUIRoomMessageTypeCustomLike) {
        if (self.onReceivedCustomLike) {
            self.onReceivedCustomLike(sender, [[AUIRoomCustomLikeModel alloc] initWithData:data]);
        }
        return;
    }
