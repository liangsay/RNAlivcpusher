...
-(void)startPlay{
    if (self.vidAuth!=nil || self.source!=nil) {
        [self.player start];
    }
}
...
- (void)setVidAuth:(NSDictionary *)vidAuth {
    _vidAuth = vidAuth;
  if (vidAuth) {
        AVPVidAuthSource *authSource = [[AVPVidAuthSource alloc] init];
        authSource.vid = vidAuth[@"videoId"];
        authSource.playAuth = vidAuth[@"playAuth"]; // 播放凭证
        authSource.region = @"cn-shanghai";
