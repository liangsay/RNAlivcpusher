//
//  WUQueenBeautyModule.m
//  RNAlivcpusher
//
//  Created by liu jinliang on 2023/11/17.
//

#import "WUQueenBeautyModule.h"
#import "NSBundle+Alivcpusher.h"
...
@interface WUQueenBeautyModule()<QueenEngineDelegate,QueenMaterialDelegate>
...
@implementation WUQueenBeautyModule
// To export a module named WUQueenBeautyModule
RCT_EXPORT_MODULE();


RCT_EXPORT_METHOD(setupBeaytyEngine) {
  __weak typeof(self) weakSelf = self;
  dispatch_async(dispatch_get_main_queue(), ^{
    [weakSelf checkModelsAndInitQueenEngine];
  });
}
...
- (void)initBeautyEngine
{
    ...
    configInfo.licenseKey = @"vPQcrgU3vwLNkoBwv083c7e6c30cf4d41a1ffbeb7b21cf2ff";
    // 相当于mainBundlePath的文件路径，示例工程的crt文件放在跟目录，所以直接填写文件名即可。
    configInfo.licenseFile = @"license.crt";
    ...
    // 设置资源根目录
  NSBundle *resBundle = [NSBundle bundleWithBundleName:@"queen_res" podName:@"AUIBeauty"];
    NSString *bundlPath = [resBundle bundlePath];
  ...
  [self onQueenBeautyType:YES];
  // 设置美白系数
  [self.beautyEngine setQueenBeautyParams:kQueenBeautyParamsWhitening value:0.3f];
  // 设置红润系数
  [self.beautyEngine setQueenBeautyParams:kQueenBeautyParamsSkinRed value:0.3f];
  // 设置磨皮系数
  [self.beautyEngine setQueenBeautyParams:kQueenBeautyParamsSkinBuffing value:0.3f];
  // 设置锐化系数
  [self.beautyEngine setQueenBeautyParams:kQueenBeautyParamsSharpen value:0.3f];
  
//    [self testBaseFaceBeauty];//磨皮
